...
- Podman (preferred) or Docker must be installed and running on your local
  machine.
...
1. **Build the Image**
   ...
   From the root of the Phlex repository, run the following command to build
   ...
   This creates a local image named `phlex-jules-dev` based on `ubuntu:24.04`,
   with a non-root user `jules` and passwordless `sudo` — a minimal,
   well-defined environment for running the preparation script.

2. **Run the Container**

   Start an interactive session inside the container, mounting your local Phlex
   repository into the container's workspace:

   ```bash
   podman run -it --rm --volume "$(pwd):/app" phlex-jules-dev /bin/bash
   ```

   You are now inside a shell in the container at the `/app` directory. Any
   changes you make to the repository files on your host machine will be
   immediately reflected inside the container.

3. **Run the Environment Preparation Script**

   From within the container, execute the script to test it:

   ```bash
   dev/jules/prepare-environment.sh
   ```

   If the script completes successfully, the environment is correctly
   configured.
