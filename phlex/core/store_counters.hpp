...
namespace phlex::experimental {
  ...
  protected:
    store_flag& flag_for(data_cell_id::hash_type hash);
    bool done_with(product_store_const_ptr const& store);

  private:
    using flags_t = tbb::concurrent_hash_map<data_cell_id::hash_type, std::unique_ptr<store_flag>>;
  ...
  private:
    using counts_t2 =
      tbb::concurrent_unordered_map<data_cell_id::hash_type, std::atomic<std::size_t>>;
  ...
  protected:
    ...
    std::unique_ptr<store_counter> done_with(data_cell_id::hash_type hash);

  private:
    using counters_t =
      tbb::concurrent_hash_map<data_cell_id::hash_type, std::unique_ptr<store_counter>>;
