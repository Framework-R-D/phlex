...
namespace phlex::detail {
  ...
  class serialized_resource_entry : public resource_base {
  public:
    using resource_type = T;
    using token_type = T::token_type;

    template <typename... Args>
    explicit serialized_resource_entry(Args&&... args) :
  ...
  private:
    ...
    {
      if constexpr (std::convertible_to<T*, token_type>) {
        return std::addressof(resource);
      } else if constexpr (std::same_as<token_type, T>) {
        return resource;
      } else {
