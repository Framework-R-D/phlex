...
namespace {
  ...
  public:
      ...
      m_type_map(type_map), m_output_file(std::move(output_file)), m_technology(technology)
    {
      ...
      // Register all products from config
      for (auto const& product_name : products) {
        output_cfg.addItem(product_name, m_output_file, m_technology);
        std::cout << "  Registered product '" << product_name << "' with FORM\n";
      }
...
PHLEX_EXPERIMENTAL_REGISTER_ALGORITHMS(m, config)
{
  ...
  // Extract products from configuration
  std::vector<std::string> products;
  try {
    // The products field is a JSON object like: {"sum": {"branch_name": "sum"}}
    // We just need the keys (product names)
    auto products_obj = config.get_object("products");
    for (auto const& key : products_obj.keys()) {
      products.push_back(key);
    }
  } catch (...) {
    std::cout << "Warning: No products configured, using empty list\n";
  }
  
  std::cout << "Products to register: ";
  for (auto const& p : products) std::cout << p << " ";
  std::cout << "\n";
  
  // Phlex needs an OBJECT
  // Create the FORM output module
  auto form_output = m.make<FormOutputModule>(type_map, output_file, technology, products);
