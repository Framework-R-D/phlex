...
namespace {
  // Provider functions
  int provide_number(data_cell_index const& index)
  {
    return 2 * static_cast<int>(index.number());
  }
  int provide_another(data_cell_index const& index)
  {
    return 3 * static_cast<int>(index.number());
  }
  int provide_still(data_cell_index const& index)
  {
    return 4 * static_cast<int>(index.number());
  }
