...
static PyObject* pcm_subscript(py_config_map* pycmap, PyObject* args)
{
  ...
  if (type == (PyObject*)&PyUnicode_Type) {
    if (!coll) {
      const auto& cvalue = pycmap->ph_config->get<std::string>(cname);
      pyvalue = PyUnicode_FromString(cvalue.c_str());
    } else {
      const auto& cvalue = pycmap->ph_config->get<std::vector<std::string>>(cname);
    ...
    if (!coll) {
      const auto& cvalue = pycmap->ph_config->get<long>(cname);
...
static PyMappingMethods pcm_as_mapping = {
    nullptr, (binaryfunc)pcm_subscript, nullptr
};
