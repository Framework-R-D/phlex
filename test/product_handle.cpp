...
TEST_CASE("Can only construct handles with compatible types (compile-time checks)", "[data model]")
{
  ...
  static_assert(std::constructible_from<handle<int>, int, data_cell_id>);
  static_assert(std::constructible_from<handle<int>, int const, data_cell_id>);
  static_assert(std::constructible_from<handle<int>, int const&, data_cell_id>);
  static_assert(not std::constructible_from<handle<int>, double, data_cell_id>);
}
...
TEST_CASE("Handle copies and moves", "[data model]")
{
  ...
  auto job_data_cell = data_cell_id::base_ptr();
...
TEST_CASE("Handle comparisons", "[data model]")
{
  ...
  handle const h17{seventeen, data_cell_id::base()};
  handle const h18{eighteen, data_cell_id::base()};
  CHECK(h17 == h17);
  CHECK(h17 != h18);

  auto subrun_6_data_cell = data_cell_id::base_ptr()->make_child(6, "subrun");
  handle const h17sr{seventeen, *subrun_6_data_cell};
  CHECK(*h17 == *h17sr);                             // Products are the same
  CHECK(h17.data_cell_id() != h17sr.data_cell_id()); // Levels are not the same
...
{
  int const number{3};
  handle const h{number, data_cell_id::base()};
  CHECK(h.data_cell_id() == data_cell_id::base());
  ...
  CHECK(handle{composer, data_cell_id::base()}->name == "Elgar");
}
