...
TEST_CASE("Two predicates", "[filtering]")
{
  ...
  g.provide("provide_num", give_me_nums, concurrency::unlimited)
    .output_product(product_query({.creator = "input"s, .layer = "event"s, .suffix = "num"s}));
  g.predicate("evens_only", evens_only, concurrency::unlimited)
    .input_family(product_query({.creator = "input"s, .layer = "event"s, .suffix = "num"s}));
...
TEST_CASE("Two predicates in parallel (each with multiple arguments)", "[filtering]")
{
  ...
  g.provide("provide_other_num", give_me_other_nums, concurrency::unlimited)
    .output_product(
      product_query({.creator = "input"s, .layer = "event"s, .suffix = "other_num"s}));
  g.predicate("evens_only", evens_only, concurrency::unlimited)
    .input_family(product_query({.creator = "input"s, .layer = "event"s, .suffix = "num"s}));
  g.predicate("odds_only", odds_only, concurrency::unlimited)
    .input_family(product_query({.creator = "input"s, .layer = "event"s, .suffix = "num"s}));
  g.make<check_multiple_numbers>(5 * 100)
    ...
    .input_family(
      product_query({.creator = "input"s, .layer = "event"s, .suffix = "num"s}),
      product_query(
        {.creator = "input"s, .layer = "event"s, .suffix = "other_num"s})) // <= Note input order
    .experimental_when("evens_only");
  ...
  g.make<check_multiple_numbers>(-5 * 100)
    ...
    .input_family(
      product_query({.creator = "input"s, .layer = "event"s, .suffix = "other_num"s}),
      product_query(
        {.creator = "input"s, .layer = "event"s, .suffix = "num"s})) // <= Note input order
    .experimental_when("odds_only");
