...
TEST_CASE("Output data products", "[graph]")
{
  ...
  g.provide("provide_number", [](data_cell_index const&) -> int { return 17; })
    .output_product(product_query{.creator = "input"_id, .layer="spill"_id, .suffix="number_from_provider"_id});

  g.transform(
     "square_number",
     [](int const number) -> int { return number * number; },
     concurrency::unlimited)
    .input_family(product_query{.creator = "input"_id, .layer="spill"_id, .suffix="number_from_provider"_id})
