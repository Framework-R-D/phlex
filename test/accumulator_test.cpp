...
namespace Catch {
  // Specialize Catch's StringMaker for the fold result type to get better test output formatting
  template <>
  struct StringMaker<std::pair<int, std::size_t>> {
    static std::string convert(std::pair<int, std::size_t> const& p)
    {
...
namespace {
  ...
  auto count_fold(internal::accumulator_message<int> const& msg) -> std::size_t
  {
    msg.partial_result->call([](int& acc) {
      ...
      // atomic_ref to safely update the count.
      std::atomic_ref(acc) += 1;
    });
