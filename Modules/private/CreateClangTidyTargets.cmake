...
# - Recursive target discovery via DIRECTORY properties (BUILDSYSTEM_TARGETS, SUBDIRECTORIES)
# - Optional inclusion of tests and FORM targets
# - Robust absolute source path resolution (SOURCE_DIR, BINARY_DIR)
# - Exclusion of generated .cxx under PROJECT_BINARY_DIR and common rootcling patterns
...
if(CLANG_TIDY_EXECUTABLE)
  ...
  if(_clang_tidy_major AND _clang_tidy_major VERSION_LESS "19")
    message(WARNING "Detected clang-tidy version ${_clang_tidy_major} (< 19). Proceeding, but 19+ is recommended.")
  endif()
endif()
...
# Prefer CMAKE_BINARY_DIR for multi-project workspaces (compile_commands.json is at the top-level),
# fall back to PROJECT_BINARY_DIR if needed.
...
function(_phlex_gather_targets_recursive out_var start_src_dir)
  set(_collected)

  # Targets defined in this directory scope
  get_directory_property(_this_dir_targets DIRECTORY "${start_src_dir}" PROPERTY BUILDSYSTEM_TARGETS)
  if(_this_dir_targets)
    list(APPEND _collected ${_this_dir_targets})
  endif()

  # Recurse into subdirectories created by add_subdirectory()
  get_directory_property(_subs DIRECTORY "${start_src_dir}" PROPERTY SUBDIRECTORIES)
...
# - excludes generated .cxx under PROJECT_BINARY_DIR and rootcling dictionary patterns
function(_phlex_collect_clang_tidy_sources out_var)
  # Limit traversal to this project's source subtree
  _phlex_gather_targets_recursive(_all_targets "${PROJECT_SOURCE_DIR}")

  # Precompute an escaped PROJECT_BINARY_DIR for regex checks
  set(_proj_bin_dir_re "${PROJECT_BINARY_DIR}")
  # Escape regex metacharacters
  string(REGEX REPLACE "([][.^$+*?()|\\])" "\\\\\\1" _proj_bin_dir_re "${_proj_bin_dir_re}")
  ...
  foreach(_t IN LISTS _all_targets)
    ...
    foreach(_s IN LISTS _sources)
      ...
      if(IS_ABSOLUTE "${_s}")
        ...
        # Fallback: REALPATH relative to SOURCE_DIR even if not yet generated
