...
function(_phlex_collect_clang_tidy_sources out_var)
  ...
  foreach(_t IN LISTS _all_targets)
    ...
    foreach(_s IN LISTS _sources)
      ...
      if(_s MATCHES [=[(/|^)G__.*\.cxx$]=] OR _s MATCHES [=[(/|^).*_([Dd]|rd)ict\.cxx$]=])
        continue()
      endif()
...
function(_create_clang_tidy_targets_impl)
  ...
  if(PHLEX_ALL_CXX_SOURCES)
    if(RUN_CLANG_TIDY_EXECUTABLE)
      ...
      set(_phlex_run_clang_tidy_sh "${CMAKE_BINARY_DIR}/phlex_run_clang_tidy_fix.sh")
      configure_file(
      ...
      add_custom_target(
        ...
        COMMAND
          "${CLANG_TIDY_EXECUTABLE}" -p "${_phlex_compile_db_dir}"
          --config-file="${_phlex_clang_tidy_config}" --fix --fix-errors ${PHLEX_ALL_CXX_SOURCES}
