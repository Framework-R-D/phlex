...
function(_create_coverage_targets_impl)
  ...
  set(
    ...
    --exclude
    [=[.*\.hxx$]=]
    # Exclude assertion macro lines: these are used for preconditions/invariants and their false
    # branches are intentionally not exercised in tests.
  ...
  if(LLVM_COV_EXECUTABLE AND LLVM_PROFDATA_EXECUTABLE)
    ...
    if(Python_FOUND AND EXISTS "${_normalize_llvm_script}")
      # When lcov is available, add an intermediate step to strip assertion branch noise before
      # normalization. Assertion macros are used for preconditions/invariants; their false
      # branches are intentionally not exercised in tests.
      ...
      if(LCOV_EXECUTABLE)
        ...
        add_custom_command(
          ...
          COMMAND
            ${CMAKE_COMMAND} -E echo
            "[Coverage] Omitting assertion macro lines from LLVM LCOV data"
          COMMAND
            ${LCOV_EXECUTABLE} -a ${LLVM_COV_LCOV_OUTPUT} --omit-lines
            "^\\s*(assert|Expects)\\s*\\(" --output-file ${LLVM_COV_LCOV_OUTPUT} --rc
            branch_coverage=1 --ignore-errors mismatch,inconsistent,negative,empty,unused
          COMMAND ${CMAKE_COMMAND} -E touch ${LLVM_COV_LCOV_FILTERED}
          WORKING_DIRECTORY ${CMAKE_BINARY_DIR}
          COMMENT "Filtering assertion branch noise from LLVM LCOV data"
          VERBATIM
        )
  ...
  if(LCOV_EXECUTABLE AND GENHTML_EXECUTABLE)
    ...
    add_custom_target(
      ...
      COMMAND
        ...
        "^\\s*(assert|Expects)\\s*\\(" --output-file coverage.info.final --rc branch_coverage=1
