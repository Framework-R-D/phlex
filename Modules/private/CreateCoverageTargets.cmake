...
if(NOT LCOV_EXECUTABLE)
    message(
        WARNING
        "lcov not found; HTML coverage reports will not be available."
    )
endif()
if(NOT GENHTML_EXECUTABLE)
    message(
        WARNING
        "genhtml not found; HTML coverage reports will not be available."
    )
endif()
if(NOT GCOVR_EXECUTABLE)
    message(
        WARNING
        "gcovr not found; XML coverage reports will not be available."
    )
endif()
...
function(create_coverage_targets)
    ...
    cmake_language(
        ...
        CALL _create_coverage_targets_impl
    )
endfunction()
...
function(_create_coverage_targets_impl)
    ...
    get_property(
        _coverage_defined
        GLOBAL
        PROPERTY _PHLEX_COVERAGE_TARGETS_DEFINED
    )
    if(_coverage_defined)
        message(
            WARNING
            "Coverage targets already defined; skipping duplicate creation."
        )
        return()
    endif()
    ...
    if(LLVM_COV_EXECUTABLE AND LLVM_PROFDATA_EXECUTABLE)
        ...
        add_custom_command(
            ...
            COMMAND
                ...
                "[Coverage] Scanning for profile data files (*.profraw)"
            COMMAND ${CMAKE_COMMAND} -E rm -f ${PROFRAW_LIST_FILE}
            COMMAND
                bash -c
                "find test -name '*.profraw' -type f -size +0c > ${PROFRAW_LIST_FILE}"
            COMMAND
                ${CMAKE_COMMAND} -E echo
        ...
        add_custom_command(
            ...
            COMMAND
                ${CMAKE_COMMAND} -E echo
                "[Coverage] Generating coverage report using llvm-cov"
            COMMAND
                ${LLVM_COV_EXECUTABLE} report ${LLVM_COV_OBJECTS}
        ...
        set(LLVM_COV_NORMALIZED_STAMP
            ${CMAKE_BINARY_DIR}/coverage-llvm-normalized.stamp
        )
        if(Python3_FOUND AND EXISTS "${_normalize_llvm_script}")
            add_custom_command(
                ...
                COMMAND
                    ${Python3_EXECUTABLE} "${_normalize_llvm_script}"
                    --repo-root "${PROJECT_SOURCE_DIR}" --coverage-root
                    "${PROJECT_SOURCE_DIR}" --coverage-alias
                    "${PROJECT_SOURCE_DIR}" "${LLVM_COV_LCOV_OUTPUT}"
                COMMAND ${CMAKE_COMMAND} -E touch ${LLVM_COV_NORMALIZED_STAMP}
                WORKING_DIRECTORY ${CMAKE_BINARY_DIR}
                COMMENT
                    "Normalizing LLVM LCOV coverage report for editor/CI tooling"
                VERBATIM
            )
            add_custom_target(
                coverage-llvm-normalize
                DEPENDS ${LLVM_COV_NORMALIZED_STAMP}
            )
        ...
        add_custom_target(
            coverage-llvm
            DEPENDS
                ${_coverage_llvm_primary_dependency}
                coverage-llvm-lcov
                ${LLVM_COV_OUTPUT}
    ...
    if(GCOVR_EXECUTABLE)
        ...
        if(EXISTS ${_gcovr_summary_binary_dir})
            list(
                APPEND _gcovr_summary_filter_paths
                ${_gcovr_summary_binary_dir}
            )
            get_filename_component(
                _gcovr_summary_binary_real
                ${_gcovr_summary_binary_dir}
                REALPATH
            )
            if(
                NOT _gcovr_summary_binary_real STREQUAL ${_gcovr_summary_binary_dir}
            )
                list(
                    APPEND _gcovr_summary_filter_paths
                    ${_gcovr_summary_binary_real}
        ...
        foreach(_gcovr_summary_filter_path IN LISTS _gcovr_summary_filter_paths)
            ...
            list(
                APPEND GCOVR_SUMMARY_FILTER_ARGS
                --filter
                "${_gcovr_summary_filter_escaped}/.*"
            )
        endforeach()
        add_custom_target(
            ...
            COMMAND
                ...
                ${GCOVR_SUMMARY_FILTER_ARGS} ${GCOVR_EXCLUDE_ARGS}
        ...
        message(
            STATUS
            "Added 'coverage-summary' target for terminal summary output."
        )
    endif()

    set(_coverage_symlink_script
        "${PROJECT_SOURCE_DIR}/scripts/create_coverage_symlinks.py"
    )
    ...
    if(Python3_FOUND AND EXISTS "${_coverage_symlink_script}")
        add_custom_target(
            ...
            COMMAND
                ${Python3_EXECUTABLE} "${_coverage_symlink_script}" --build-root
                "${CMAKE_BINARY_DIR}" --output-root "${_coverage_symlink_root}"
            WORKING_DIRECTORY ${CMAKE_BINARY_DIR}
        ...
        add_custom_target(
            ...
            COMMAND
                ${CMAKE_COMMAND} -E make_directory "${_coverage_symlink_root}"
            COMMAND
                ${CMAKE_COMMAND} -E echo
    ...
    set(_normalize_xml_script
        "${PROJECT_SOURCE_DIR}/scripts/normalize_coverage_xml.py"
    )
    set(_normalize_lcov_script
        "${PROJECT_SOURCE_DIR}/scripts/normalize_coverage_lcov.py"
    )
    ...
    add_custom_target(
        coverage
        COMMAND
            echo "Generating coverage reports from existing coverage data..."
        WORKING_DIRECTORY ${CMAKE_BINARY_DIR}
        COMMENT
    ...
    if(LCOV_EXECUTABLE AND GENHTML_EXECUTABLE)
        ...
        add_custom_target(
            coverage-html
            COMMAND
                ${LCOV_EXECUTABLE} --directory . --capture --output-file
                coverage.info --rc branch_coverage=1 --ignore-errors
                mismatch,inconsistent,negative --ignore-errors deprecated
            COMMAND
                ${LCOV_EXECUTABLE} --remove coverage.info
                ${LCOV_REMOVE_PATTERNS} --output-file coverage.info.cleaned --rc
            ...
            COMMAND
                ${GENHTML_EXECUTABLE} -o coverage-html coverage.info.final
                --title "Phlex Coverage Report" --show-details --legend
                --branch-coverage --ignore-errors
        ...
        if(Python3_FOUND AND EXISTS "${_normalize_lcov_script}")
            add_custom_target(
                ...
                COMMAND
                    ${Python3_EXECUTABLE} "${_normalize_lcov_script}"
                    --repo-root "${PROJECT_SOURCE_DIR}" --coverage-root
                    "${PROJECT_SOURCE_DIR}" --coverage-alias
                    "${PROJECT_SOURCE_DIR}"
                ...
                WORKING_DIRECTORY ${CMAKE_BINARY_DIR}
                COMMENT
                    "Normalizing LCOV HTML coverage report for editor/CI tooling"
                VERBATIM
            )
    ...
    if(GCOVR_EXECUTABLE)
        ...
        if(Python3_FOUND AND EXISTS "${_normalize_xml_script}")
            add_custom_target(
                ...
                COMMAND
                    ${Python3_EXECUTABLE} "${_normalize_xml_script}" --repo-root
                    "${PROJECT_SOURCE_DIR}" --source-dir "${PROJECT_SOURCE_DIR}"
                    --path-map
        ...
        set(_coverage_gcov_dependencies
            coverage-symlink-prepare
            coverage-xml
            coverage-summary
        )
